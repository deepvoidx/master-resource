...
document.getElementById('add-tool-btn').addEventListener('click', function(){
  var fab = this;
  fab.classList.remove('water');
  // Force reflow so animation restarts even on repeated clicks
  void fab.offsetWidth;
  fab.classList.add('water');
  fab.addEventListener('animationend', function(){ fab.classList.remove('water'); }, { once:true });
  openToolModal(-1);
});
...
document.querySelectorAll('.tab-btn').forEach(function(btn){
  btn.addEventListener('click', function(){
    ...
    S.activeTab = tab;
    // Toggle FAB visibility — only show on Tools tab
    var panel = document.getElementById('admin-panel');
    if (tab === 'tools') panel.classList.add('tab-tools-active');
    else panel.classList.remove('tab-tools-active');
...
function applyPreviewDevice(device) {
  ...
  // Full reset before applying new device
  ...
  if (device === 'mobile') {
    ...
    // Desktop: read container width NOW (frame is reset to auto width)
    // then set frame to 1280px and scale to fit — do NOT read containerW after setting width
    frame.style.borderRadius = '12px';
    var DESKTOP_W = 1280;
    var containerW = wrap.getBoundingClientRect().width;
    if (!containerW || containerW <= 0) containerW = DESKTOP_W;
    var scale = Math.min(1, containerW / DESKTOP_W);
    frame.style.width = DESKTOP_W + 'px';
    // Apply scale after paint — use offsetHeight (reflects computed CSS height, not inline)
    requestAnimationFrame(function(){
      frame.style.transform = 'scale(' + scale + ')';
      frame.style.transformOrigin = 'top left';
      var frameH = frame.offsetHeight || 600;
