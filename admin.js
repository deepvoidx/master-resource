...
(function(){
  ...
  if (!catFab) return;
  catFab.addEventListener('click', function(){
    catFab.classList.remove('water-pop');
    void catFab.offsetWidth;
    catFab.classList.add('water-pop');
    catFab.addEventListener('animationend', function onEnd(){
      catFab.classList.remove('water-pop');
      catFab.removeEventListener('animationend', onEnd);
    });
    openCatModal(-1);
  });
})();
...
function doLogin() {
  ...
  Promise.race([validateGHToken(pat), timeoutP])
    .then(function(ok){
      ...
      if (!ok) {
        recordFailedAttempt();
        errEl.textContent='Incorrect token. Please try again.';
        btn.disabled=false; btn.textContent='Sign In';
        return;
      }
      return getGHUser(pat).then(function(user){
        return onLoginSuccess(pat, user);
      });
    })
    .catch(function(e){
