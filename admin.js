...
function scheduleWithUndo(msg, actionFn, onSuccess, onRevert) {
  ...
  document.getElementById('ut-msg').textContent = msg;
  el.classList.add('show');
  prog.style.transition = 'none'; prog.style.width = '100%';
  requestAnimationFrame(function(){ requestAnimationFrame(function(){
    prog.style.transition = 'width '+UNDO_MS+'ms linear'; prog.style.width = '0%';
  }); });
...
document.getElementById('ut-undo').addEventListener('click', function(){
  cancelUndo(); toast('Action cancelled', '↩️');
});
document.getElementById('ut-close').addEventListener('click', function(){
  cancelUndo(); toast('Action cancelled', '↩️');
});
...
function openToolModal(idx) {
  ...
  categories.forEach(function(cat){
    ...
    lbl.className = 'cat-check' + (tool.category === cat.id ? ' checked':'');
    lbl.dataset.cid = cat.id;
    lbl.innerHTML = '<input type="checkbox" value="'+esc(cat.id)+'"'+(tool.category===cat.id?' checked':'')+'/>'
                  + esc(cat.icon)+' '+esc(cat.short||cat.label);
    lbl.addEventListener('click', function(){
      catsEl.querySelectorAll('.cat-check').forEach(function(l){ l.classList.remove('checked'); l.querySelector('input').checked=false; });
...
document.getElementById('tm-save').addEventListener('click', function(){
  ...
  var catEl = document.querySelector('#tm-cats .cat-check.checked input');
  if (!catEl){ errEl.textContent='Select a category.'; return; }
  var catId = catEl.value;

  var tags = tagsRaw.split(',').map(function(t){ return t.trim().replace(/^#/,'').toLowerCase(); }).filter(Boolean);
  var tool = { name:nameVal, description:desc, url:urlRes.url, category:catId, tags:tags };
...
document.querySelectorAll('.sort-btn-sm').forEach(function(btn){
  btn.addEventListener('click', function(){
    ...
    S.toolsSortMode = btn.getAttribute('data-sort');
...
function renderStats() {
  ...
  if (!S.toolsData) { el.innerHTML='<div class="empty-state"><div>Load data first.</div></div>'; return; }
